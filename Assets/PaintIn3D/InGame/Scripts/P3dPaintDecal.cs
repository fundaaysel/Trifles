...
namespace PaintIn3D
{
    ...
    public class P3dPaintDecal : MonoBehaviour, IHit, IHitPoint, IHitLine, IHitTriangle, IHitQuad, IHitCoord
    {
        ...
        {
            set { shapeChannel = value; }
            get { return shapeChannel; }
        }
        ...
        {
            set { normalFront = value; }
            get { return normalFront; }
        }
        ...
        {
            set { normalBack = value; }
            get { return normalBack; }
        }
        ...
        {
            set { normalFade = value; }
            get { return normalFade; }
        }
        ...
        public float TileOpacity
        {
            ...
            get { return tileOpacity; }
        }

        [UnityEngine.Serialization.FormerlySerializedAs("tileBlend")] [Range(0.0f, 1.0f)] [SerializeField]
        ...
        {
            set { tileTransition = value; }
            get { return tileTransition; }
        }
        ...
        public P3dModifierList Modifiers
        {
            get
            {
                if (modifiers == null) modifiers = new P3dModifierList();
                return modifiers;
            }
        }
            ...
            Quaternion rotation)
        {
            ...
            P3dPaintableManager.SubmitAll(P3dCommandDecal.Instance, worldPosition, worldRadius, layers, group,
                targetModel, targetTexture);
        }

        /// <summary>This method paints all pixels between the two specified points using the shape of a decal.</summary>
        public void HandleHitLine(bool preview, int priority, float pressure, int seed, Vector3 position,
        ...
        public void HandleHitTriangle(bool preview, int priority, float pressure, int seed, Vector3 positionA,
            Vector3 positionB, Vector3 positionC, Quaternion rotation)
        {
            P3dCommandDecal.Instance.SetState(preview, priority);
        ...
        public void HandleHitQuad(bool preview, int priority, float pressure, int seed, Vector3 position,
            Vector3 endPosition, Vector3 position2, Vector3 endPosition2, Quaternion rotation)
        {
            P3dCommandDecal.Instance.SetState(preview, priority);
        ...
        public void HandleHitCoord(bool preview, int priority, float pressure, int seed, P3dHit hit,
            Quaternion rotation)
        {
        ...
        private Vector3 HandleHitCommon(bool preview, float pressure, int seed, Quaternion rotation)
        {
            if (!workOneTime)
            {
                ...
                P3dCommandDecal.Instance.SetMaterial(blendMode, finalTexture, shape, shapeChannel, finalHardness,
                    wrapping,
                    normalBack, normalFront, normalFade, finalColor, finalOpacity, tileTexture, finalMatrix,
                    tileOpacity,
                    tileTransition);
...
namespace PaintIn3D
{
    ...
    public class P3dPaintDecal_Editor : P3dEditor
    {
        protected override void OnInspector()
        {
            ...
            EndError();
            EditorGUILayout.PropertyField(serializedObject.FindProperty("shapeChannel"), GUIContent.none,
                GUILayout.Width(50));
            ...
            if (DrawFoldout("Advanced", "Show advanced settings?") == true)
            {
                ...
                Draw("normalFront",
                    "This allows you to control how much the paint can wrap around the front of surfaces (e.g. if you want paint to wrap around curved surfaces then set this to a higher value).\n\nNOTE: If you set this to 0 then paint will not be applied to front facing surfaces.");
                Draw("normalBack",
            ...
            tgt.Modifiers.DrawEditorLayout(serializedObject, target, "Color", "Angle", "Opacity", "Radius", "Scale",
                "Hardness", "Texture", "Position");
        }
    }
}
#endif